...
contract Pitch is ERC1155Supply, Ownable {
    ...
    function setTokenCost(uint256 _tokenCost, uint256 tokenId) external onlyOwner {
        tokenCost[tokenId] = _tokenCost;
    }

    function setAllTokenCosts(uint256 _tokenCostA, uint256 _tokenCostB, uint256 _tokenCostC, uint256 _tokenCostD) external onlyOwner {
        tokenCost[TOKEN_ID_A] = _tokenCostA;
        tokenCost[TOKEN_ID_B] = _tokenCostB;
        tokenCost[TOKEN_ID_C] = _tokenCostC;
        tokenCost[TOKEN_ID_D] = _tokenCostD;
    }
    ...
    function mintTokenWl(uint256 _amount, uint256 tokenId) external payable {
        require(saleIsActiveWl, "wl inactive");
        require(msg.value > tokenCost[tokenId] * _amount, "sent too little eth");
        require(_amount <= , "MUST_MINT_AT_LEAST_ONE_TOKEN");
